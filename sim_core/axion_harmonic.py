...
class SimulationConfig:
    ...
    track_first_collision: bool = False
...
class AxionSimulation:
    ...
    def _periodic_distance(self, p1: np.ndarray, p2: np.ndarray) -> float:
        delta = p1 - p2
        box = self.config.boxSize_comoving
        for dim in range(3):
            delta[dim] -= box * round(delta[dim] / box)
        return float(np.sqrt(np.dot(delta, delta)))

    def _try_record_first_collision(self, tau: float,
                                    centers: np.ndarray,
                                    radii: np.ndarray):
        if self.first_collision_recorded or not self.config.track_first_collision:
            return
        n = len(radii)
        if n < 2:
            return

        tol = self.config.collision_tolerance
        a_tau = self.cosmology.scale_factor(tau)

        for i in range(n - 1):
            r_i = float(radii[i])
            for j in range(i + 1, n):
                r_j = float(radii[j])
                dist = self._periodic_distance(centers[i], centers[j])
                if dist <= (r_i + r_j + tol):
                    self.simulation_data['first_collision'] = {
                        'tau': float(tau),
                        't_cosmic': float(self.cosmology.cosmic_time_from_conformal(tau)),
                        'bubble_i': int(i),
                        'bubble_j': int(j),
                        'radius_i_comoving': r_i,
                        'radius_j_comoving': r_j,
                        'radius_i_physical': float(r_i * a_tau),
                        'radius_j_physical': float(r_j * a_tau),
                        'center_distance_comoving': dist,
    ...
    def run_simulation(self, tau_final: float, save_interval: int = 10,
        ...
        try:
            ...
            while tau < tau_final:
                ...
                self._try_record_first_collision(tau_new, centers_end, radii_end)
